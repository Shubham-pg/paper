...
# Global distance fields for safety evaluation
...
if node_s in G and node_g in G:
    try:
        ...
        k_shortest_nodes = []
        accepted_paths_data = [] # Stores (node_path, total_weight) for filtering
        
        # --- NEW: Macroscopic Path Filter ---
        for node_path in path_generator:
            # 1. Calculate this path's total length
            path_weight = sum(G[node_path[i]][node_path[i+1]]['weight'] for i in range(len(node_path)-1))
            
            # 2. Extract unique edges and weights for comparison
            ...
            for acc_path, acc_weight in accepted_paths_data:
                ...
                # Check how much length this path shares with an already accepted path
                shared_length = sum(w for e, w in edges_curr.items() if e in edges_acc)
                
                # If they share too much distance, it's just a micro-variation. Reject it!
        ...
        # --- PHASE 3: SHRINK-WRAP LOCAL TRIMMING ---
        path_mask = np.ones_like(mask)
        for r, c in best_path['pixels']:
            path_mask[int(r), int(c)] = 0
            
        dist_to_path, closest_path_idx = distance_transform_edt(path_mask, return_indices=True)
        
        core_rigid_sdf = sdf_rigid[closest_path_idx[0], closest_path_idx[1]]
        core_soft_sdf = sdf_soft[closest_path_idx[0], closest_path_idx[1]]
        
        dynamic_thresh_rigid = np.minimum(CLEAR_RIGID, core_rigid_sdf + dist_to_path)
        dynamic_thresh_soft = np.minimum(CLEAR_SOFT, core_soft_sdf + dist_to_path)
        ...
        cv2.putText(final_vis, "Notice how overlapping bands locally shrink-wrapped!", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
