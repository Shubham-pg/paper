...
NODE_REST_DIST = 5.0        # (Pixels) Ideal spring length. 
...
STABILITY_THRESHOLD = 0.02  # (Pixels) Max movement per frame to be considered "settled"
STABLE_FRAMES_REQ = 30      # Consecutive frames under threshold required to Auto-Stop
# -------------------------------------------
...
print("Starting Auto-Stabilizing Spring-Mass Simulation...")
...
while True:
    ...
    if popping_active and frame % POP_INTERVAL == 0 and frame > 0:
        ...
        if len(P) > POP_STRIDE * 2:
            ...
            # Failsafe: Rope is perfectly straight and out of nodes, stop popping
    ...
    for i in range(1, len(P)-1):
        ...
        # Apply Net Force
        movement = W_SPRING * tension + W_REPEL * repel
        P_new[i] += movement
        
        # Track maximum movement for Auto-Stop logic
        move_dist = np.hypot(movement[0], movement[1])
