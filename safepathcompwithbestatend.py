    ...
    try:
        ...
        for node_path in path_generator:
            ...
            if is_unique:
                ...
                print(f"  > Found distinct path {len(accepted_paths_data)} (Length: {path_weight:.1f}px)")
                
            if len(accepted_paths_data) >= K_PATHS:
                break
                
        paths_info = []
        for i, (node_path, path_weight) in enumerate(accepted_paths_data):
            hwy_pixel_path = []
            ...
            paths_info.append({
                ...
                'weight': round(path_weight, 1) # Track path length for logical comparisons
            })
        ...
        # --- PHASE 2: CHOOSE SAFEST PATH & DETERMINE REASON ---
        safe_paths = [p for p in paths_info if not p['has_overlap']]
        if safe_paths:
            best_path = safe_paths[0] 
            best_reason = f"No obstacle overlap & Shortest valid length ({best_path['weight']}px)"
            print(f"\nWINNER: Path {best_path['id']}")
            print(f"Reason: {best_reason}")
        else:
            best_path = max(paths_info, key=lambda p: p['lex_score'])
            worst_choke = best_path['lex_score'][0] if best_path['lex_score'] else "N/A"
            best_reason = f"Maximized clearance at tightest squeeze (Worst Choke: {worst_choke}px)"
            print(f"\nWINNER: Path {best_path['id']}")
            print(f"Reason: {best_reason}")
        ...
        cv2.putText(final_vis, f"WINNER: Path {best_path['id']}", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(final_vis, f"Reason: {best_reason}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        cv2.putText(final_vis, "Notice how ONLY intersected walls were selectively trimmed!", (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
