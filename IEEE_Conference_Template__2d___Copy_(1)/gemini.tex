...
% ==========================================
\begin{figure}
\centerline{\includegraphics[width=\columnwidth]{Designer.png}}
...
\label{eq:lex}
\end{equation}
The first component is the minimum clearance, so \eqref{eq:lex} first maximizes the tightest passage --- the standard bottleneck objective, on which the two criteria agree exactly. The refinement acts on ties: when candidates share a tightest passage the order proceeds to the second-tightest.  Comparison is $O(M)$ with early exit. Fig.~\ref{fig:lexicographical} shows an enumerated candidate set and the selected route.
...
\end{tabular}
\end{center}
\footnotesize Band stage outputs an executable smoothed trajectory. Routing row is the like-for-like global-planning comparison. $^{\dagger}$ Normalized per environment against A$^\star$ attainability, our mean feasibility is $99.6\%$ (Table~\ref{tab:perenv}). Dashes (---) indicate metrics omitted due to prohibitive execution time (FMM batch), topological failure (GVD maze), or inapplicability (band smoothing in search comparisons).
\end{table*}
