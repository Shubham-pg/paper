...
% The Topological Cage: Search-Free, Clearance-First
% Routing for Mobile Robots

The Topological Cage: Search-Free, Diverse-Path Routing with Guaranteed Clearance for Mobile Robots
}
...
Mobile robots in cluttered indoor environments repeatedly answer start-to-goal queries on a largely static map. Grid search (A$^\star$, Dijkstra) is resolution-complete but expands cells in proportion to free area, and because it minimizes length, it hugs obstacle boundaries. Sampling planners (PRM, RRT$^\star$) scale better in dimension, but narrow doorways and non-convex traps demand high sampling densities that erode this advantage. 

Distance-field skeletons invert this paradigm: precompute the locus of maximum clearance once, then route on it. This gives safety by construction and yields a routing graph orders of magnitude smaller than the grid~\cite{lau2013,oleynikova2018,qi2025egvg}. Table~\ref{tab:requirements} summarizes how standard planner families fare against the requirements of repeated querying on a fixed map.



Despite their advantages, two problems prevent skeletal representations from serving as a standalone planning layer. First is the \textbf{injection problem}: query points almost never lie exactly on the skeleton. Connecting them via local grid search reintroduces expansion costs that scale poorly in open space, while alternatives like nearest-pixel projection compromise safety. Second is the \textbf{Voronoi void}: standard medial axes often fragment or become sparse in expansive rooms or around free-standing islands, breaking the routing structure (detailed further in Appendix~\ref{appendix:voronoi_void}).



Both defects share a root cause: the usable skeleton is an \emph{unbounded, possibly disconnected} set. Our response makes the routing structure \emph{bounded and connected by construction}. A tangent--gradient test identifies the structured interior and supplies the clearance scale $C_{\max}$; the ridge is truncated at $d_{\mathrm{cage}}=C_{\max}+\Delta$ and unioned with the distance level set at that value. The cut ends lie exactly on the level set, fastening its components into one cohesive structure. Map processing is paid once; each query then guarantees injection without search, allowing the residual budget to fund the enumeration of topologically distinct routes selected by clearance.

\textbf{Contributions.} (i) A bounded medial manifold from a tangent--gradient level-selection rule and a single super-critical level set, with a proof of connectedness, raising feasibility from $24.0\%$ to $99.6\%$. (ii) A search-free bidirectional gradient injection attaching $100\%$ of terminals with zero connector collisions. (iii) A lexicographic bottleneck criterion refining min-clearance selection, cutting ties by $56.7\%$. (iv) A damped spring--mass band with per-obstacle relaxation, reducing integrated curvature by $85.2\%$.
...
% \begin{figure*}[htbp]
% \centerline{\includegraphics[width=0.96\textwidth]{hand_drawn_robot.png}}
% \caption{Pipeline execution in complex environments. \textbf{Far Left:} Hardware executing global routing around free-standing obstacles. \textbf{Center Left \& Right:} Simulated concave islands and a dense maze. The cage (magenta) unifies the skeleton, and the lexicographic criterion selects the optimal route (green) from $K=5$ candidates. Semantic clearances: blue denotes soft obstacles (lower offset), red denotes rigid (larger band). \textbf{Far Right:} Physical execution of the damped band (rope tensioner) settling the trajectory. \emph{Validated across 10 maps, showing flawless enumeration, selection, and selective relaxation in simulation and reality.}}
% \label{fig:casestudy}
% \end{figure*}
...
\caption{Planner families against the requirements of repeated querying on a fixed map.}
\label{tab:requirements}
\begin{tabular}{@{}l ccccc@{}}
\toprule
\textbf{Family} & \textbf{R1} & \textbf{R2} & \textbf{R3} & \textbf{R4} & \textbf{R5} \\
\midrule
Grid search~\cite{hart1968,koenig2005}   & \no  & \yes & \no  & \no  & \no \\
Sampling~\cite{kavraki1996,karaman2011}  & \partially & \partially & \no & \partially & \no \\
GVD / GVG~\cite{lau2013,oleynikova2018}  & \no  & \no  & \yes & \yes & \no \\
EGVG~\cite{qi2025egvg}                   & \partially & \partially & \no & \yes & \partially \\
\rowcolor{black!7}
Topological Cage (ours)                  & \yes & \yes & \yes & \yes & \yes \\
\bottomrule
\addlinespace[2pt]
\multicolumn{6}{@{}p{0.96\columnwidth}@{}}{\scriptsize
\textbf{R1} attach any terminal without grid search.
\textbf{R2} one connected structure across open regions.
\textbf{R3} select by clearance, not length.
\textbf{R4} build cost amortized over queries.
\textbf{R5} executable path with a per-obstacle margin.
\yes\ met, \partially\ met conditionally, \no\ not met. Sampling meets R1, R2 and
R4 at a density fixed before the queries are known, and RRT$^\star$ is
single-query; EGVG attaches by raycast, so R1 follows vertex placement, and its
bands carry no connectedness guarantee.}
\end{tabular}
\end{table}
...
\begin{figure*}[htbp]
\centerline{\includegraphics[width=0.70\textwidth]{Designer.png}}
...
Two properties matter. $\mathcal{M}$ is a deformation retract of $\Omega$, hence \emph{connected} whenever $\Omega$ is. But it is \emph{unbounded in clearance}: it follows local clearance maxima into every open region, out to the widest parts of the map (Fig.~\ref{fig:medial_combined}, Left). The portion planners retain --- the structured corridors --- is what fragments into disconnected components.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\columnwidth]{infinitemedial.png}%
\hfill
\includegraphics[width=0.49\columnwidth]{angleprunning.png}
\caption{\textbf{Left (Medial ridge):} The skeleton (cyan) captures interior corridors correctly but branches into open space toward global maxima. \emph{The skeleton is connected but unbounded in clearance; retaining only its structured part fragments it.} \textbf{Right (Tangent--gradient test):} Interior and inter-island corridors show near-orthogonal tangent/gradient pairs ($\theta\!\approx\!8^\circ$--$9^\circ$). Exterior branches diverge ($\theta\!\approx\!110^\circ$) and are excluded from $\mathcal{M}_{\mathrm{int}}$ by a single threshold.}
\label{fig:medial_combined}
\end{figure}

\subsection{Tangent--Gradient Test and Level Selection}

Inside a corridor the medial curve runs parallel to the flanking walls, so the gradient is near-orthogonal to the skeleton tangent; on a branch radiating into open space this breaks down. With $\theta(p)=\bigl|\,90^\circ-\arccos\bigl|\hat{t}_{\mathcal{M}}(p)\cdot\hat{n}(p)\bigr|\,\bigr|$, the structured interior is
\begin{equation}
...
\begin{equation}
{\ \mathcal{C}=\mathcal{M}_{\mathrm{trunc}}\ \cup\ \mathcal{B}\ }
\label{eq:cage}
\end{equation}
...
\begin{figure}[htbp]
\centerline{%
  \includegraphics[width=0.49\columnwidth]{scout_navigation_map.png}\hfill%
  \includegraphics[width=0.49\columnwidth]{pathcomp.png}%
}
\caption{\textbf{Left:} Search-free injection. The climber (green) integrates $+\hat{n}$, the diver (orange) $-\hat{n}$, until contact with the cage (purple). Which branch succeeds is set by whether the terminal lies below or above the cage level. \textbf{Right:} Homotopy enumeration and lexicographic selection. Panels show $K$ topologically distinct candidates; annotations give the sorted clearance profile head. The selected route maximizes the tightest passage first and the second-tightest next. \emph{Selection is by safety profile, not length.}}
\label{fig:merged_scouts_lex}
\end{figure}
...
\label{eq:lex}
\end{equation}
The first component is the minimum clearance, so \eqref{eq:lex} first maximizes the tightest passage --- the standard bottleneck objective, on which the two criteria agree exactly. The refinement acts on ties: when candidates share a tightest passage the order proceeds to the second-tightest.  Comparison is $O(M)$ with early exit. Fig.~\ref{fig:merged_scouts_lex} (Right) shows an enumerated candidate set and the selected route.
...
Discretizing $P^\star$ into unit-mass nodes $\{P_i\}$ with fixed endpoints,
\begin{equation}
...
\label{eq:repel}
\end{equation}
so a node outside a threshold feels no force from that component. While $\Psi_O$ is an exact distance field where mathematically $\|\nabla\Psi_O\| \equiv 1$, the explicit normalization in \eqref{eq:repel} is retained in implementation to robustify against discrete approximation errors on the grid. 
...
\textbf{Environments.} Four $500\times750$~px benchmarks at $0.02$~m/px ($150$~m$^2$): \textbf{E1} (structured indoor, mixed obstacles), \textbf{E2} (semi-sparse, Voronoi-void stress case), \textbf{E3} (free-standing islands), and \textbf{E4} (U-shaped trap). Obstacles carry rigid/deformable labels with distinct thresholds. Twenty different complex operator-drawn and physical maps serve as case studies to validate real-world and simulated performance.
...
\textbf{Ours} (global routing)      & $343.9$ & $24.7$ & $\mathbf{2.81}$ & $92.8^{\dagger}$ & & $\mathbf{98}$ & $\mathbf{14.0}$ & $\mathbf{28.38}$ & \textbf{yes} \\
\textbf{Ours} (routing $+$ band)       & $343.9$ & $84.7$ & $8.81$ & $92.8$ & & --- & --- & --- & --- \\ \bottomrule
\end{tabular}
\end{center}
\footnotesize \textbf{Total 100q (s)}: Amortized batch time $= (\text{Offline} + 100 \times \text{Query}) / 1000$. \textbf{Routing}: Like-for-like global planning comparison. \textbf{Band}: Outputs executable smoothed trajectories. $^{\dagger}$Normalized against A$^\star$ attainability (mean $99.6\%$). Dashes (---): Omitted due to timeouts (FMM), topological failure (GVD), or inapplicability.
...
The most dramatic improvements align with theoretical expectations: in open (E2) and free-standing (E3) maps, uncaged feasibility is $7\%$ and $13\%$, while the cage hits $100\%$. In E1, sealed walls cap theoretical attainability at $72\%$; our planner resolves $71\%$. Across all $400$ queries, our method missed only one attainable path, giving $99.6\%$ mean normalized feasibility. Query times show enumeration dominates ($22.13$~ms) compared to injection ($2.16$~ms). Graph sizes ($44$--$116$ vertices) scale with topology, not map area.

\begin{table}[htbp]
\caption{Per-environment closure, feasibility and query cost}
\label{tab:perenv}
...
Total/Mean & $21$ & $\mathbf{5}$ & $93$ & $24.0$ & $\mathbf{92.8}$ & $2.16$ & $22.13$ & $53.57$ \\ \bottomrule
\end{tabular}

\vspace{3pt}
\footnotesize Graph sizes $|V|/|E|$: E1 $116/163$, E2 $52/75$, E3 $75/114$, E4 $44/61$. Normalized feasibility (caged $\div$ attainable, per environment) is $98.6/100/100/100$, mean $99.6\%$. Costs exclude overheads from Table~\ref{tab:sweep}.
\end{table}
...
\textbf{Selection and band.} Lexicographic and max-min-clearance criteria yield identical minimum clearance ($1.001$~m), outperforming shortest-length selection by $3.0\%$. Lexicographic refinement further reduces ties ($247\rightarrow107$) and improves mean clearance ($1.557\rightarrow1.588$~m). The full elastic band cuts integrated curvature by $85.2\%$ ($26.96\rightarrow3.98$) and spacing dispersion by $97.4\%$ ($3.810\rightarrow0.101$) relative to the raw skeletal route. Damping is critical: omitting it spikes curvature $39\times$ and leaves only $2.5\%$ of runs settled. Removing selective relaxation raises curvature by $37.7\%$ ($3.98\rightarrow5.48$) and drops settling from $83.8\%$ to $67.5\%$. 

In the E4 trap ($33$ trials), the myopic DWA-style planner failed $33/33$ times, while both RRT$^\star$ and our pipeline succeeded $33/33$. However, unlike RRT$^\star$, which returns any valid homotopy class (often grazing boundaries), our method strictly guarantees route selection via clearance profiles.
...
% \begin{tabular}{@{}llccc@{}}
% \toprule
% \textbf{ID} & \textbf{Variant} & \textbf{Metric 1} & \textbf{Metric 2} & \textbf{Metric 3} \\ \midrule \multicolumn{5}{@{}l}{\textit{A1 --- closure (feasibility \%, min clr m, query ms)}} \\
...
\begin{tabular}{llccc}
\toprule
\textbf{ID} & \textbf{Variant} & \textbf{Metric 1} & \textbf{Metric 2} & \textbf{Metric 3} \\
\midrule
\multicolumn{5}{l}{\textit{A1 --- closure (feasibility pct, min clr m, query ms)}} \\
A1 & no closure & $26.3$ & $0.910$ & $45.4$ \\
\midrule
\multicolumn{5}{l}{\textit{A2 --- injection (attach pct, safety/cost, time ms)}} \\
A2a & nearest-pixel & $100.0$ & $2.1$ collide & $0.879$ \\
A2b & A$^\star$ to skeleton & $100.0$ & $2443$ cells & $3.182$ \\
A2c & raycast~\cite{qi2025egvg} & $98.4$ & $\mathbf{0}$ collide & $\mathbf{0.079}$ \\
-- & \textbf{gradient (ours)} & $\mathbf{100.0}$ & $\mathbf{0}$ collide & $0.727$ \\
\midrule
\multicolumn{5}{l}{\textit{A3 --- scouts (attach pct, min clr m, routed pct)}} \\
A3 & climber only & $93.4$ & $0.870$ & $83.8$ \\
A3 & diver only & $9.1$ & --- & $0.0$ \\
-- & \textbf{both} & $\mathbf{100.0}$ & $0.854$ & $\mathbf{94.4}$ \\
\midrule
\multicolumn{5}{l}{\textit{A4 --- selection (min clr m, mean clr m, ties)}} \\
A4 & shortest length & $0.972$ & $1.530$ & $1$ \\
A4 & max min-clr & $\mathbf{1.001}$ & $1.557$ & $247$ \\
-- & \textbf{lexicographic} & $\mathbf{1.001}$ & $\mathbf{1.588}$ & $\mathbf{107}$ \\
\midrule
\multicolumn{5}{l}{\textit{A5 --- band (curvature, spacing CV, settled pct)}} \\
A5 & raw skeletal & $26.96$ & $3.810$ & --- \\
A5 & no damping & $155.11$ & $0.648$ & $2.5$ \\
A5 & no relaxation & $5.48$ & $0.185$ & $67.5$ \\
A5 & end decimation & $3.94$ & $0.174$ & $\mathbf{86.9}$ \\
A5 & no rest length & $\mathbf{2.77}$ & $0.153$ & $70.6$ \\
-- & \textbf{full} & $3.98$ & $\mathbf{0.101}$ & $83.8$ \\
\bottomrule
\end{tabular}
\end{center}

\vspace{-3pt}
\footnotesize Collision entries are percentages. A5 curvature is $\int|\kappa|\,ds$. A3 per-environment attachment: climber $100/73.8/100/100$, diver $1.3/\mathbf{27.5}/3.8/3.8$, either $100$ throughout; only E2 places terminals above $d_{\mathrm{cage}}$ in quantity. Gradient injection takes $60.7$ integration steps per terminal.
\end{table}
...
Fig.~\ref{fig:casestudy} demonstrates the pipeline combining real-world physical executions and simulated complex environments. The framework was evaluated across 20 different maps, functioning perfectly in both simulation and on physical hardware. The real-world hardware experiments showcase the omnidirectional robot executing both the initial global routing and the final settled trajectory via the damped band (rope tensioner) routine. The simulations highlight two geometrically complex cases with high branching, where the cage unifies the skeleton into a single routing manifold, terminals inject seamlessly, and the lexicographic criterion successfully selects a winner from $K=5$ candidates.
...
% In a large open region the structured-corridor interpretation of the medial axis degenerates: the nearest boundaries are far apart and the equidistance locus is sparse, offering no usable routing structure at the scale a robot needs (Fig.~\ref{fig:voronoi_void_resolution}, Left). Around a free-standing obstacle the failure differs in kind --- the skeleton forms a closed loop \emph{around} the obstacle, a legitimate structure with no corridor-level connection to the rest. Both are measurable: in E2 the uncaged structure splits into four components and resolves $7\%$ of queries; in E3, five islands, six components and $13\%$.

% The standard recourse is a grid search until a skeleton pixel is found, whose cost grows with the size of the void --- precisely the wrong scaling, confirmed by the $3{,}068$ versus $2{,}031$ expansion asymmetry between E2 and E1. The cage removes the need: $\mathcal{B}$ passes through the open region by construction because $\Psi$ attains $d_{\mathrm{cage}}$ there, and gradient integration reaches that level within arc length $|d_{\mathrm{cage}}-\Psi(p_0)|$ (Fig.~\ref{fig:voronoi_void_resolution}, Right).

% \begin{figure}[htbp]
% \centering
% \includegraphics[width=0.49\columnwidth]{voronoi_shortcoming_raw.png}%
% \hfill
% \includegraphics[width=0.49\columnwidth]{voronoi_our_path.png}
% \caption{\textbf{Left (Voronoi Void):} The standard medial axis (red) provides no usable structure across the open region, leaving the query point (green) with nothing to connect to. \emph{The failure is in the representation, not the connection procedure.} \textbf{Right (Resolution by closure):} The cage (purple) crosses the open region; the scout reaches it by monotone gradient integration and the query routes on the unified structure (yellow) with no grid expansion.}
% \label{fig:voronoi_void_resolution}
% \end{figure}
...
In large open regions, the medial axis degenerates into a sparse locus devoid of robot-scale routing structure (Fig.~\ref{fig:voronoi_void_resolution}, Left). Around free-standing obstacles, it similarly fails by forming disconnected loops. This fragmentation is measurable: uncaged routing resolves just $7\%$ of queries across four components in E2 (open space), and $13\%$ across six components in E3 (islands).

Connecting terminals typically requires a grid search that scales poorly with void size ($3{,}068$ vs.\ $2{,}031$ expansions in E2 vs.\ E1). The cage removes this overhead: $\mathcal{B}$ structurally spans open regions because $\Psi$ attains $d_{\mathrm{cage}}$ there. Consequently, gradient integration guarantees connection to this level within arc length $|d_{\mathrm{cage}}-\Psi(p_0)|$ (Fig.~\ref{fig:voronoi_void_resolution}, Right).
...
\hfill
\includegraphics[width=0.49\columnwidth]{voronoi_our_path.png}
\caption{\textbf{Left (Voronoi Void):} The standard medial axis (red) leaves open regions empty, stranding the query (green). \emph{The defect is representational, not procedural.} \textbf{Right (Resolution by closure):} The cage (purple) spans the region, enabling search-free connection via monotone gradient integration and routing on a unified structure (yellow).}
\label{fig:voronoi_void_resolution}
\end{figure}
