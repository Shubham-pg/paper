...
For localized map changes, only edges whose pixel chains intersect the changed region need recomputation; a full cage rebuild is not required. This incremental path is \emph{not implemented in the current prototype}, and no incremental-update timings are claimed.
% ==========================================
...
With $p_S'$ and $p_G'$ inserted, we enumerate $K$ loopless shortest paths on $G$ by Yen's algorithm~\cite{yen1971}, discarding candidates whose shared edge weight with an accepted candidate exceeds a fraction of the shorter path, so the retained set spans distinct homotopy classes rather than perturbations of one class. We do not claim topological completeness of the enumeration.
...
\label{eq:relax}
\end{equation}
where $\tau_O^{\mathrm{nom}}$ is the class-dependent nominal threshold and $\eta$ a small margin. Components not on the route keep $\tau_O^{\mathrm{nom}}$. The clamp at $\rho+\delta_{\mathrm{safe}}$ preserves the safety guarantee: relaxation never goes below the robot radius plus margin. Eq.~\eqref{eq:relax} is evaluated once per query, before tensioning. Removing it (Table~\ref{tab:kinematics}) raises integrated curvature by $37.6\%$ and drops the settled fraction from $83.8\%$ to $67.5\%$.
...
\label{sec:impl}
We distinguish the method as formulated from the current proof-of-concept, so prototype choices are not read as design claims.
\textbf{Implemented.} Exact EDT and central-difference gradients; ridge extraction and thinning; tangent--gradient pruning by local-PCA tangent estimation; $C_{\max}$ computation and single-level-set closure; degree-based graph abstraction with junction clustering; bidirectional gradient injection; Yen $K$-shortest enumeration with a macroscopic distinctness filter; lexicographic bottleneck selection; connected-component obstacle labelling with per-component distance fields; selective relaxation; semi-implicit damped spring--mass band with distributed decimation and kinetic auto-stop.
\textbf{Prototype-specific choices.} $\tau$ and $\Delta$ are fixed scalars; $\Delta$ is expressed in pixels and is therefore resolution-dependent. The sweeps in Sec.~\ref{sec:ablation} show feasibility is invariant to both over wide ranges, so this is a convenience rather than a tuned dependence. Semantic obstacle classes are assigned from a hand-authored map layer rather than from perception. Stub trimming length $\ell_{\min}$ is fixed.
\textbf{Specified but not implemented.} Incremental cage update under local map changes; automatic selection of $\Delta$ from robot radius and map statistics; multi-agent shared-cage scheduling. No results depend on these.
Parameters used in all reported runs are in Table~\ref{tab:params}.
\begin{table}[htbp]
\caption{Parameters used in reported experiments}
\label{tab:params}
\begin{center}
\begin{tabular}{@{}llll@{}}
\toprule
...
\textbf{Environments.} Four benchmark maps, each $500\times750$ px at $0.02$ m/px ($10.0\times15.0$ m, $150$ m$^2$). \textbf{E1}: structured indoor layout with rooms, doorways and soft furniture; free space is itself partitioned, so at most $72\%$ of randomized pairs are connected. \textbf{E2}: semi-sparse map with a large open region, the Voronoi-void stress case. \textbf{E3}: five free-standing obstacle islands with no continuous interior walls. \textbf{E4}: U-shaped non-convex trap with a single $22$-px exit corridor.
...
\textbf{Metrics.} Offline preprocessing time; per-query time split into injection / enumeration / selection / tensioning; injection success and connector-collision rate; connected components before and after closure; query feasibility; minimum and mean path clearance; path length; integrated absolute curvature $\int|\kappa|\,ds$ and angular jerk (RMS third difference of heading); node-spacing coefficient of variation; settling iterations; graph $|V|,|E|$.
...
\textbf{Note on feasibility.} In E1 the free space is genuinely partitioned by sealed room walls, and no planner can exceed $72\%$. Feasibility figures are therefore reported as measured; the reachable ceiling is $93.0\%$ averaged across the four maps.
% ==========================================
% IX. RESULTS
% ==========================================
\section{Results}
\label{sec:results}
\subsection{Multi-Query Cost}
...
Two baselines are faster in raw wall time but pay for it elsewhere. GVD with A$^\star$ terminal connection completes queries in $17.4$~ms but declares only $33.5\%$ feasible, because its skeleton is disconnected. EGVG-style raycast injection is the fastest attachment mechanism at $0.5$~ms but reaches $90.0\%$ feasibility and, as Table~\ref{tab:injection} shows, fails outright on $6.5\%$ of terminals in E4. Our method is the only entry that matches the reachable feasibility ceiling of $93.0\%$.
The full pipeline including band optimization costs $84.7\pm60.2$~ms. We report both rows because the smoothing stage produces an executable trajectory that none of the baselines generate; the $24.7$~ms row is the like-for-like global-routing comparison.
\begin{table}[htbp]
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{Offline} & \textbf{Query} & \textbf{Total} & \textbf{Feas.} \\
 & (ms) & (ms) & (s) & (\%) \\ \midrule
...
\textbf{Ours} (routing) & $\mathbf{343.9}$ & $\mathbf{24.7}$ & $\mathbf{2.81}$ & $\mathbf{93.0}$ \\
\textbf{Ours} (+ band) & $343.9$ & $84.7$ & $8.81$ & $93.0$ \\ \bottomrule
\end{tabular}
\end{center}
\end{table}
\begin{table}[htbp]
...
\end{center}
\end{table}
The breakdown locates the cost precisely. Injection is $2.16$~ms and selection $0.38$~ms; enumeration at $22.13$~ms dominates the routing stage, and the $K$-sweep of Sec.~\ref{sec:ablation} shows clearance saturates at $K=3$, so this is reducible. Graph sizes of $45$--$115$ vertices on $375{,}000$-pixel maps confirm that $|V|$ tracks topology rather than area.
An independent single-query maze benchmark run on a separate map (Table~\ref{tab:maze}) corroborates the expansion-count argument: our method visits $98$ nodes where A$^\star$ visits $244{,}021$ and FMM $360{,}000$, while attaining the highest clearance of any method tested.
\begin{table}[htbp]
...
\end{center}
\end{table}
\subsection{Connectivity and Injection}
Table~\ref{tab:connectivity} isolates the effect of closure, and it is the strongest result in the paper. Summed over the four maps the raw pruned skeleton splits into $21$ components; after adding $\mathcal{B}$ this falls to $5$, and E2--E4 collapse to a single component each. The consequence for planning is direct: mean feasibility rises from $24.0\%$ to $92.8\%$. The failure is most extreme exactly where the theory predicts --- in the open-room map E2 the uncaged planner solves $7\%$ of queries and the caged planner solves $100\%$, and in the island map E3 the figures are $13\%$ and $100\%$. E1 retains two cage components because its free space is genuinely partitioned; the caged planner reaches $71$ of the $72$ attainable queries.
\begin{table}[htbp]
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Env} & \textbf{Comp.} & \textbf{Comp.} & \textbf{Feas.} & \textbf{Feas.} \\
 & \textbf{raw} & \textbf{caged} & \textbf{raw (\%)} & \textbf{caged (\%)} \\ \midrule
E1$^{\dagger}$ & $8$ & $2$ & $25.0$ & $71.0$ \\
E2 & $4$ & $\mathbf{1}$ & $7.0$ & $\mathbf{100.0}$ \\
E3 & $6$ & $\mathbf{1}$ & $13.0$ & $\mathbf{100.0}$ \\
E4 & $3$ & $\mathbf{1}$ & $51.0$ & $\mathbf{100.0}$ \\ \midrule
Total/Mean & $21$ & $\mathbf{5}$ & $24.0$ & $\mathbf{92.8}$ \\ \bottomrule
\end{tabular}
\end{center}
\footnotesize $^{\dagger}$E1 free space is partitioned; $72\%$ is the attainable ceiling.
\end{table}
Table~\ref{tab:injection} compares attachment mechanisms over $800$ randomized terminals. Gradient injection is the only mechanism achieving both $100\%$ success and zero connector collisions. Nearest-pixel projection matches the success rate but routes $2.1\%$ of connectors through obstacles, confirming that geometric projection is unsafe. The A$^\star$ fallback is safe and complete but expands $2{,}443$ cells per terminal --- $41.5\times$ our step count --- and is $4.4\times$ slower. Raycast injection is fastest but drops to $93.5\%$ in E4, where vertex placement leaves terminals without line of sight.
\begin{table}[htbp]
...
\end{figure}
\subsection{Clearance and Trap Resolution}
...
\end{center}
\end{table}
In the E4 trap environment ($33$ trials, start inside the U, goal outside), the myopic local planner was captured in $33/33$ trials, never escaping the concavity. RRT$^\star$ and our pipeline both resolved $33/33$. The distinction between the two is not success but guarantee: our route is selected by clearance profile over enumerated classes, whereas the sampling planner returns whichever class it happens to find, and the maze case study of Table~\ref{tab:maze} shows that class can graze obstacles.
...
\end{center}
\end{table}
The auto-stop rule triggered before the iteration cap in $83.8\%$ of runs, at $389\pm419$ iterations.
\textbf{Hardware validation.} Physical trials on a differential-drive TurtleBot~3 under overhead motion capture with floor projection of the cage, scouts and live band are \PH{in progress; N trials per environment}. All results in this section are planner-side geometric measurements.
% ==========================================
% X. ABLATIONS
% ==========================================
...
\textbf{A3 --- Bidirectional vs.\ single scout.} The climber branch alone attaches $83.8\%$ of terminals. The diver branch alone attaches none, since descending toward the boundary rarely intersects the cage before violating the safety floor. Used together as a best-of-two, the pair reaches $94.4\%$: the diver contributes $10.6$ percentage points in exactly the configurations where the climber stalls on a plateau, which is why we retain it.
\textbf{A6 --- Parameter sensitivity.} Table~\ref{tab:sweep} sweeps the four free parameters. Feasibility is invariant at $91.3\%$ across $\tau\in[8^\circ,45^\circ]$ and $\Delta\in[5,80]$~px, with graph size varying by under $9\%$ and mean clearance by under $7\%$. This is the result that answers the obvious reviewer objection: the prototype constants are not tuned, because the method does not depend on them within any reasonable range. Clearance saturates at $K=3$, so the enumeration budget can be cut without loss. Injection succeeds on $100\%$ of terminals for $\alpha\in[0.25,2.0]$~px with step counts falling from $127.5$ to $17.2$; only at $\alpha=4.0$ does the integration overshoot the cage and degrade to $87.5\%$. Running at $\alpha=2.0$ would reduce injection cost by a further $3.7\times$ at no measured loss.
\begin{table}[htbp]
...
Both failures are measurable. In E2, a semi-sparse map with one large open region, the raw skeleton splits into four components and the resulting planner solves $7\%$ of randomized queries. In E3, built from five free-standing islands, it splits into six components and solves $13\%$.
\begin{figure}[htbp]
