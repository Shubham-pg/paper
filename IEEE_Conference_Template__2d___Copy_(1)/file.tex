...
Distance-field skeletons invert this: precompute the locus of maximum clearance once, then route on it. This gives clearance by construction and a graph orders of magnitude smaller than the grid~\cite{lau2013,oleynikova2018,qi2022pruned}. Recent work makes the representation practical online, including incremental updates~\cite{lau2013} and, closest to our setting, the Enveloped Generalized Voronoi Graph (EGVG)~\cite{qi2025egvg}, which augments the Voronoi skeleton with distance-field contour bands. Two problems remain.

\textbf{The injection problem.} A query point almost never lies on the skeleton. Nearest-pixel projection is brittle and can cut through obstacles --- measured here at $2.1\%$ of terminals. Inserting the query as a temporary obstacle forces a local skeleton rebuild. Running A$^\star$ or BFS until a skeleton pixel is hit reintroduces grid expansion, and its cost grows with distance to the skeleton, which is exactly the open-space case that dominates. Raycasting toward nearby vertices~\cite{qi2025egvg} is fast but not guaranteed, and its success depends on vertex placement.
...
\label{eq:ridge}
\end{equation}
with $N(p)$ the $8$-neighborhood, thinned to one pixel. Eq.~\eqref{eq:ridge} is standard and not claimed as a contribution.

Two properties matter. $\mathcal{M}$ is a deformation retract of $\Omega$, hence \emph{connected} whenever $\Omega$ is. But it is \emph{unbounded in clearance}: it follows local clearance maxima into every open region, out to the widest parts of the map. The portion planners actually retain --- the structured corridors --- is what fragments into disconnected components.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.90\columnwidth]{infinitemedial.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.95\columnwidth]{medialplusband.png}}
...
This is the formal content of P2: the cage does not merely enclose the interior structure, it unifies it.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{pathcomp.png}}
...
\caption{Band optimization. Left: the selected skeletal route, clearance-maximal but jagged. Right: the settled band, resting against selectively relaxed thresholds at the bottlenecks instead of being repelled out of them, with uniform node spacing.}
...
\textbf{Implementation and parameters.} Python with NumPy, OpenCV and NetworkX; exact EDT~\cite{felzenszwalb2012} and Yen's routine~\cite{yen1971}; single-threaded on \PH{CPU model, RAM}. Parameters are fixed across every environment and run: $\tau=17^\circ$, $\Delta=20$~px, $\alpha=0.5$~px, $K=5$, max.\ overlap $0.85$, $r=0.02$~m/px, $\rho+\delta_{\mathrm{safe}}=6$~px ($0.12$~m), $\tau^{\mathrm{nom}}_{\mathrm{rigid}}=30$~px, $\tau^{\mathrm{nom}}_{\mathrm{soft}}=15$~px, $\eta=2$~px, $d_{\mathrm{rest}}=5$~px, initial spacing $8$~px, $W_s=0.55$, $W_r=2.5$, $D=0.4$, $h=1.0$, $\epsilon_{\mathrm{stab}}=0.7$~px/iter, $n_{\mathrm{hold}}=30$, $(N_{\mathrm{pop}},T_{\mathrm{pop}})=(10,20)$, iteration cap $1500$.

\textbf{Benchmark suite.} Four maps, $500\times750$ px at $0.02$ m/px ($150$~m$^2$). \textbf{E1}: structured indoor layout with rooms, doorways, soft furniture. \textbf{E2}: semi-sparse with a large open region (Voronoi-void stress case). \textbf{E3}: five free-standing islands, no continuous interior walls. \textbf{E4}: U-shaped non-convex trap with a single $22$-px exit. Obstacles carry rigid/deformable class labels with distinct nominal thresholds. Three additional operator-drawn maps of substantially higher geometric complexity --- nested spiral corridors, islands with concave cavities, and a dense multi-branch maze --- serve as qualitative case studies.

\textbf{Resolution criterion.} A query is \emph{resolved} if at least one enumerated candidate admits a band solution whose per-component relaxed thresholds all remain above $\rho+\delta_{\mathrm{safe}}$. If every candidate would require relaxation below that floor --- the only passages are narrower than the robot plus margin --- the query is reported \emph{unresolved} rather than executed. This is a deliberate refusal to traverse an unsafe gap, not a search failure, and it is monotone in $K$. All baselines are scored against the same floor.

\textbf{Baselines.} A$^\star$ on the metric grid~\cite{hart1968}; PRM with $2500$ samples, $k=12$, density tuned to the narrowest doorway~\cite{kavraki1996}; RRT$^\star$~\cite{karaman2011} with goal bias $0.05$; GVD routing with A$^\star$ terminal connection; a myopic DWA-style local planner~\cite{fox1997} for the trap study. A$^\star$ also serves as reachability oracle: a pair is \emph{attainable} if A$^\star$ solves it, which normalizes feasibility against the connectivity of free space itself. \emph{We do not reimplement EGVG}; we evaluate its terminal-connection mechanism --- raycasting to the $k$ nearest graph vertices --- on our own structure, and report it as an injection ablation (Sec.~\ref{sec:results}) rather than as a competing planner.

\textbf{Protocol.} $Q=100$ randomized start--goal pairs per environment ($400$ total), sampled uniformly over free cells with $\Psi>\rho+\delta_{\mathrm{safe}}$ and separated by $\ge 120$~px, identical across methods. Injection statistics use $200$ additional terminals per environment; per-branch attachment uses $320$ terminals. Band ablations use $40$ queries per environment. Selection statistics use the $375$ queries yielding $\ge 2$ distinct candidates.

\textbf{Hardware.} An omnidirectional (omniwheel) platform tracked by overhead motion capture, with the occupancy map, cage, scouts and live band projected onto the laboratory floor. Planned paths are published as \texttt{nav\_msgs/Path} and tracked by a holonomic velocity controller; because the base is omnidirectional, the planar geometric path is directly executable without a kinematic conversion stage.
...
Table~\ref{tab:master} consolidates the evaluation. The left block reports cost and feasibility over $400$ randomized queries on E1--E4; the right block is an independent single-query maze benchmark where clearance and node counts can be compared directly across all methods.

Three points stand out. First, feasibility separates the methods more sharply than time: GVD with A$^\star$ terminal connection is fast per query ($17.4$~ms) but resolves only $33.5\%$, because its skeleton is disconnected --- it is quick at answering the wrong question. Second, our global routing stage costs $24.7$~ms against A$^\star$'s $250.2$~ms, a $10.1\times$ reduction, against a one-time offline cost of $343.9$~ms, so break-even occurs after two queries and the $100$-query total falls from $25.02$~s to $2.81$~s. Third, the maze block shows the expansion-count argument at the extreme: our method visits $98$ graph nodes where A$^\star$ visits $244{,}021$ cells and FMM $360{,}000$, while attaining $14$~px of clearance --- the only method to both succeed and stay off the obstacle boundary.

PRM is the one baseline competitive on query time ($8.2$~ms), but it pays $945.5$~ms offline for a sampling density that still resolves only $84.3\%$, and its maze clearance of $3$~px reflects sampling that does not respect corridor structure.

\begin{table*}[htbp]
\caption{Consolidated comparison. Left: cost and feasibility over $400$ randomized queries on E1--E4. Right: independent single-query maze benchmark. Best value per column in bold.}
\label{tab:master}
\begin{center}
...
\end{tabular}
\end{center}
\footnotesize The band stage produces an executable smoothed trajectory that no baseline generates; the routing row is the like-for-like global-planning comparison. Resolved counts a query only if the criterion of Sec.~\ref{sec:setup} is met; normalized against A$^\star$ attainability our figure is $99.6\%$ (Table~\ref{tab:perenv}).
\end{table*}

\subsection{Per-Environment Evidence}

Table~\ref{tab:perenv} gives the environment-level picture behind those aggregates, and it contains the central result of the paper. Summed over four maps the uncaged routing structure splits into $21$ components; after closure this falls to $5$, with E2--E4 collapsing to a single component each, as Theorem~\ref{thm:connected} predicts. Feasibility rises from $24.0\%$ to $92.8\%$ raw, and to $99.6\%$ normalized by attainability.

The effect is largest exactly where the theory predicts. In the open-room map E2 the uncaged planner resolves $7\%$ and the caged planner every attainable query; in the island map E3 the figures are $13\%$ and $100\%$. E1 is the one map whose free space is itself partitioned by sealed walls, capping any planner at $72\%$; the cage reaches $71$ of those $72$. Across all $400$ queries the caged planner leaves a single attainable query unresolved.

The timing columns also locate the cost. Injection is $2.16$~ms and selection $0.38$~ms; enumeration at $22.13$~ms dominates the routing stage. Graph sizes of $44$--$116$ vertices confirm $|V|$ tracks topology, not area.

\begin{table}[htbp]
\caption{Per-environment evidence: closure, feasibility, graph size, per-stage query cost}
\label{tab:perenv}
\begin{center}
...
\end{tabular}
\end{center}
\footnotesize Graph sizes $|V|/|E|$: E1 $116/163$, E2 $52/75$, E3 $75/114$, E4 $44/61$. Normalized feasibility (caged $\div$ attainable) is $98.6/100/100/100$, mean $99.6\%$. Stage costs exclude the graph-copy and terminal-attachment overhead included in the sweep totals of Table~\ref{tab:sweep}.
\end{table}
...
\textbf{Closure (A1)} is necessary, not incidental: routing on the truncated skeleton alone drops feasibility to $26.3\%$.

\textbf{Injection (A2).} Gradient injection is the only mechanism achieving both $100\%$ attachment and zero connector collisions. Nearest-pixel projection matches the success rate but routes $2.1\%$ of connectors through obstacles, confirming that geometric projection is unsafe. The A$^\star$ fallback is safe and complete but expands $2{,}443$ cells per terminal where we take $60.7$ integration steps, and its cost grows with distance to the skeleton --- $3{,}068$ expansions in the open map E2 against $2{,}031$ in the structured map E1, precisely the wrong scaling. Raycast injection, the mechanism EGVG uses, is the fastest at $0.079$~ms but attaches only $98.4\%$, dropping to $93.5\%$ in E4 where vertex placement leaves terminals without line of sight.

\textbf{Bidirectional scouts (A3).} The measurement confirms the partition of Theorem~\ref{thm:injection} directly. In E1, E3 and E4 the climber attaches every terminal and the diver registers $1.3$--$3.8\%$, all at a single integration step --- terminals already in contact with the cage --- because essentially none lies above the cage level. In the open-room map E2 this inverts: the climber attaches $73.8\%$ and the diver $27.5\%$, summing to $101.3\%$ against a union of $100.0\%$, so the branches overlap on only $1.3\%$ of terminals and partition the rest almost exactly. The diver's mean of $74.6$ steps there confirms a genuine descent rather than contact termination. Bidirectionality is not redundancy: it is the mechanism covering the open-space case, which is exactly the case the Voronoi void makes common.

\textbf{Selection (A4).} The lexicographic order and max-min-clearance agree exactly on minimum clearance ($1.001$~m), as theory requires, and both beat shortest-length selection by $3.0\%$. The refinement acts where it should: ties fall from $247$ to $107$, a $56.7\%$ reduction, and mean route clearance rises from $1.557$~m to $1.588$~m, for $7.3\%$ additional length --- the cost the criterion is designed to accept.

\textbf{Band (A5).} Relative to the raw skeletal route the full band cuts integrated curvature by $85.2\%$ ($26.96\rightarrow3.98$), angular jerk by $74.8\%$, and node-spacing dispersion by $97.4\%$ ($3.810\rightarrow0.101$), while shortening the path by $20.3\%$. Damping is decisive: removing it raises curvature by $39\times$, inflates length from $11.04$ to $85.49$~m as the band flails between opposing walls, and leaves only $2.5\%$ of runs settled. Selective relaxation contributes a $37.6\%$ curvature reduction and raises the settled fraction from $67.5\%$ to $83.8\%$. Distributed decimation halves spacing dispersion against end decimation at equal curvature. The rest-length term acts on spacing uniformity ($0.153\rightarrow0.101$), which is the property it is introduced to control. Auto-stop triggered before the cap in $83.8\%$ of runs at $389\pm419$ iterations.

In the E4 trap ($33$ trials, start inside the U), the myopic local planner was captured $33/33$, never escaping the concavity; RRT$^\star$ and our pipeline both resolved $33/33$. The distinction is not success but guarantee --- our route is selected by clearance profile over enumerated classes, whereas the sampling planner returns whichever class it finds, and the maze block of Table~\ref{tab:master} shows that class can graze the boundary.
...
Table~\ref{tab:sweep} sweeps the four free parameters. Feasibility is invariant at $91.3\%$ across $\tau\in[8^\circ,45^\circ]$ and $\Delta\in[5,80]$~px --- a factor of $5.6$ in the angular threshold and $16$ in the offset --- with graph size varying under $8\%$ and mean clearance under $8\%$. This answers the obvious objection to fixed constants: within the admissible range of Assumption~\ref{asm:setting} the construction does not depend on them. Clearance saturates at $K=3$ and is identical for $K\in\{3,5,8,12\}$, so the enumeration budget can be cut by $40\%$ without loss. Injection attaches $100\%$ across the whole range $\alpha\in[0.25,4.0]$~px with step count falling monotonically from $123.6$ to $9.1$; running at $\alpha=4.0$ would reduce integration cost $6.9\times$ at no measured loss.
...
\subsection{Complex Environments and Hardware}

Fig.~\ref{fig:casestudy} shows the pipeline on the three operator-drawn maps, which exceed the benchmark suite in corridor count, branching factor and boundary curvature. In each case the cage unified the skeleton into a single routing manifold, both terminals injected without search, $K=5$ distinct candidates were enumerated, and the lexicographic criterion returned a winner; all three resolved under the criterion of Sec.~\ref{sec:setup}. Selective relaxation is directly visible as locally thinned clearance shells on exactly the components the route contacts, while others retain their nominal thresholds.

Fig.~\ref{fig:hardware} shows the settled band executed on the omnidirectional platform. Because the base is holonomic the planar path is tracked without a kinematic conversion stage, so the executed trajectory is the planned band up to tracking error.

\begin{figure*}[htbp]
\centerline{\includegraphics[width=0.96\textwidth]{E134.png}}
\caption{Pipeline on three operator-drawn environments of substantially higher geometric complexity than the benchmark suite. Left: nested spiral corridors with mixed rigid (grey/red) and deformable (blue) semantic boundaries. Centre: five free-standing islands with concave cavities. Right: dense multi-branch maze. In all three the cage (magenta) unifies the skeleton into one routing manifold, both terminals inject without grid expansion, $K=5$ distinct homotopy classes are enumerated, and the lexicographic criterion selects the green route. Clearance shells are thinned only on components the route actually contacts. \emph{Enumeration, selection and per-component relaxation behave as specified well beyond the benchmark suite.}}
\label{fig:casestudy}
\end{figure*}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{hardware_omniwheel_collage.png}}
\caption{Hardware execution on the omnidirectional platform, multiple viewpoints of the same run. The occupancy map, Topological Cage and settled band are projected onto the laboratory floor, giving direct visual correspondence between the planner state and the physical workspace; the robot tracks the band through the relaxed bottleneck without heading reversal. \PH{N trials per environment; tracking error to be reported.} \emph{The executed trajectory is the planned band: with a holonomic base no kinematic conversion stage intervenes between planner output and motion.}}
\label{fig:hardware}
\end{figure}
...
\textbf{The cage level is a single global scalar.} $d_{\mathrm{cage}}$ derives from $C_{\max}$, a supremum over the whole structured interior, and the envelope is placed at that one level everywhere. A single unusually wide hallway raises $C_{\max}$ and pushes the envelope outward across the entire workspace, including regions where a tighter contour would serve better. This is fundamental to the formulation, and it has two consequences: it sets the admissibility condition of Assumption~\ref{asm:setting} --- if the tangent--gradient test admits enough open-space structure, $C_{\max}$ approaches the global maximum, $\mathcal{B}$ empties, and the construction degenerates to the bare truncated skeleton --- and it is why the cage is a static-map construct, since any edit changing the widest interior corridor changes the envelope everywhere. A spatially varying cage level, a family of contours indexed by local clearance statistics, is the natural next formulation and would address both at once.

\textbf{Lexicographic selection is decided by a single pixel.} The global minimum $\Psi_{(1)}$ determines the outcome unless two candidates tie exactly, so one aliasing artifact on a diagonal wall can flip the ordering between routes of substantially different quality. The criterion inherits the brittleness of any bottleneck objective on discretized data. Quantile-based comparison, or smoothing the profile before sorting, would trade the exact bottleneck guarantee for robustness; we have not characterized that trade-off.

\textbf{The safety floor is enforced on nodes, not chords.} Nodes are clamped to $\Psi\ge\rho+\delta_{\mathrm{safe}}$, but the segment joining consecutive nodes is unchecked. With $d_{\mathrm{rest}}$ comparable to the floor, a chord across a convex corner can dip below it while both endpoints satisfy it. The collision-free property holds for the node set, not the interpolated trajectory; closing the gap needs a swept-segment test in the clamping step.

\textbf{The disc footprint is load-bearing.} $\Psi$ equals clearance only for a circular robot. For a rectangular or non-convex footprint the equivalence fails at every stage --- level selection, clearance-profile selection and per-component relaxation all assume it. Extension requires configuration-space distance fields or a conservative inscribed-disc treatment, with loss of tightness in narrow passages. Velocity and acceleration limits are likewise not enforced in the band; the platform's holonomic base removes the kinematic conversion problem but not the dynamic one.

\textbf{Narrow-passage refusal.} When every candidate would require relaxation below the floor, the planner declines. This is intended --- the passage is genuinely narrower than the robot plus margin --- but no path is returned where a kinodynamically aggressive planner might squeeze through.

\textbf{Future work: node-level homotopy enumeration.} Yen's algorithm generates successors by removing individual edges and re-running a shortest-path search, after which candidates are filtered for macroscopic distinctness. This is wasteful in a specific way: because the cage graph is already a topological abstraction, candidates differing only in which pixel chain they traverse between the same junctions are not meaningfully distinct, yet Yen's procedure generates them and the filter discards them. A better enumeration would operate on the vertex sequence --- having fixed an optimal route, generate the next by suppressing junction vertices rather than edges, so each successor belongs to a different homotopy class by construction. The naive realization, re-running Dijkstra with each junction deleted, costs more than Yen's per iteration, but the cage graph is unusually favourable: it is small, its vertices are genuine topological branch points, and its edges carry precomputed clearance profiles. An enumeration exploiting this would produce a topologically complete, non-redundant candidate set while removing the stage that currently dominates query cost. We regard this as the most promising direction the representation opens.

% ==========================================
\section{Conclusion}

We presented the Topological Cage, a bounded medial manifold built by truncating the medial axis at a clearance level fixed by a tangent--gradient test and unioning it with the distance level set at that value. The construction is connected by proof rather than observation: the truncation points of the ridge lie on the level set and pin its components together, so the cage unifies the routing structure instead of merely enclosing it. Empirically this reduces components from $21$ to $5$ across four benchmarks and raises feasibility from $24.0\%$ to $99.6\%$ of attainable, leaving one attainable query unresolved over $400$. Because the distance field is eikonal, injection is a transversal crossing rather than a convergence, attaching $100\%$ of terminals with zero connector collisions in $60.7$ steps against $2{,}443$ expansions for the search-based fallback. The resulting cost structure breaks even against A$^\star$ after two queries and funds homotopy enumeration with lexicographic clearance selection, plus a damped band that cuts curvature by $85.2\%$ while retaining a hard safety floor. Three complex operator-drawn environments and hardware trials on an omnidirectional platform confirm the behaviour, and parameter sweeps show feasibility invariant across wide ranges of $\tau$, $\Delta$, $K$ and $\alpha$. The clearest opening is enumeration: generating successor routes by suppressing junction vertices of the cage graph, rather than edges of a pixel-level path, would make the candidate set topologically complete and, given how small the graph is, likely cheaper than the procedure used here.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{voronoi_shortcoming_raw.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.8\columnwidth]{voronoi_our_path.png}}
